...
1. [总览：5 层架构全景](#1-总览5-层架构全景)
2. [第 1 层：表现层（Presentation Layer）](#2-第-1-层表现层presentation-layer)
3. [第 2 层：业务逻辑层（Business Layer）](#3-第-2-层业务逻辑层business-layer)
4. [第 3 层：AI 智能体层（AI Agent Layer）](#4-第-3-层ai-智能体层ai-agent-layer)
5. [第 4 层：数据层（Data Layer）](#5-第-4-层数据层data-layer)
6. [第 5 层：基础设施层（Infrastructure Layer）](#6-第-5-层基础设施层infrastructure-layer)
7. [技术选型矩阵](#7-技术选型矩阵)
8. [部署架构图（生产级）](#8-部署架构图生产级)
9. [数据流全链路](#9-数据流全链路)
10. [安全与合规架构](#10-安全与合规架构)
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                      QuantInsight Pro - 5 层架构                     │
│                  AI 驱动的另类数据量化投研平台 V3.0                    │
└─────────────────────────────────────────────────────────────────────┘
                                  │
        ┌─────────────────────────┼─────────────────────────┐
        │                         │                         │
...
└───────┬───────┘         └───────┬───────┘         └───────┬───────┘
        ...
        ╚═══════════════════════════════════════════════════╝
                                  ...
                                  ▼
        ╔═══════════════════════════════════════════════════╗
        ║   第 3 层：AI 智能体层  5 大子智能体                  ║
        ║   选股Agent / 回测Agent / 风控Agent / 报告Agent / 答辩Agent ║
        ╚═══════════════════════════════════════════════════╝
                                  │
                                  ▼
        ╔═══════════════════════════════════════════════════╗
        ║   第 4 层：数据层  17 因子 + 6 类另类数据            ║
        ║   akshare / tushare / 东财 / 卫星 / 供应链 / 舆情      ║
        ╚═══════════════════════════════════════════════════╝
                                  │
                                  ▼
        ╔═══════════════════════════════════════════════════╗
        ║   第 5 层：基础设施层  阿里云双活 + 自建 GPU         ║
...
| # | 模块 | 代码量 | 关键能力 | 性能指标 |
|---|------|--------|----------|----------|
| 1 | 智能选股 | 280 行 | 17 因子 + 6 维雷达图 + Top 50 排行 | < 1.2s/股 |
| 2 | AI 投研问答 | 420 行 | Qwen3-72B + RAG + 5 大子智能体 | < 4.0s/问 |
| 3 | 量化策略回测 | 380 行 | 5 年回测 + 11 类指标 + 11.4 年实盘 | < 30s/5年 |
| 4 | SHAP 可解释性 | 180 行 | TreeExplainer + 瀑布图 + 雷达图 | < 0.8s/股 |
...
## 4. 第 3 层：AI 智能体层（AI Agent Layer）

```
┌──────────────────────────────────────────────────────────────────┐
│  第 3 层：AI 智能体层（AI Agent Layer）                             │
│  职责：5 大子智能体协同 + LLM 调用 + RAG 知识检索                    │
└──────────────────────────────────────────────────────────────────┘
...
## 5. 第 4 层：数据层（Data Layer）

```
┌──────────────────────────────────────────────────────────────────┐
│  第 4 层：数据层（Data Layer）                                     │
│  职责：17 因子计算 + 6 类另类数据接入 + 实时行情                     │
└──────────────────────────────────────────────────────────────────┘

   ┌─────────────────────────────────────────────────────────────┐
   │   17 因子多维评分引擎（6 维 × N 因子）                        │
   │   ─────────────────────────────────────                    │
...
## 6. 第 5 层：基础设施层（Infrastructure Layer）
...
| **回测引擎** | 自研 | V2 | 事件驱动 + 多策略 | Backtrader / Zipline |
...
| 用途 | 模型 | 来源 | 参数 | 成本 | 性能 |
|------|------|------|------|------|------|
| 主力 LLM | Qwen3-72B-Instruct | 阿里云百炼 | 72B | ¥0.04/千tokens | 95% |
...
└──────────────────────────────────────────────────────────────────┘
 ...
 ① 数据采集              ② 数据清洗              ③ 因子计算
 ─────────              ─────────              ─────────
 akshare/tushare/        缺失值/异常值/          XGBoost 17因子
 东财/卫星/供应链         重复值/格式              6维评分
        │                     │                     │
        ▼                     ▼                     ▼
...
> **文档结束 · QuantInsight Pro 项目技术栈架构图 V1.0**
>
